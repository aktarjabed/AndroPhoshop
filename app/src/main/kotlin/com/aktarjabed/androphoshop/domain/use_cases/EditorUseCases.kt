package com.aktarjabed.androphoshop.domain.use_cases
...
import com.aktarjabed.androphoshop.features.editor.processors.CropProcessor
import com.aktarjabed.androphoshop.features.editor.processors.FilterProcessor
...
data class EditorUseCases @Inject constructor(
    ...
    val removeBackgroundMlKit: RemoveBackgroundMlKitUseCase,
...
class RemoveBackgroundMlKitUseCase @Inject constructor(
    private val backgroundRemoverMlKit: BackgroundRemoverMlKit
) {
    suspend operator fun invoke(bitmap: Bitmap): Result<Bitmap> = withContext(Dispatchers.IO) {
        try {
            Result.Success(backgroundRemoverMlKit.removeBackground(bitmap))
        } catch (e: Exception) {
            Result.Error(e)
        }
    }
